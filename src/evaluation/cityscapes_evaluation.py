...
def evaluate(conf):
    ...
    assert conf['evaluation_mode'] in ["mono", "stereo"], "Please choose mono or stereo evaluation by setting evaluation_mode to 'mono' or 'stereo'!"

    if conf['numpy_disparities_to_evaluate'] is None: # optional path to a .npy disparities file to evaluate.

        # assert os.path.isdir(conf['pretrained_models_folder']), "Cannot find a folder at {}".format(conf['pretrained_models_folder'])

        # print("-> Loading weights from {}".format(conf['pretrained_models_folder']))
        ...
        dataset = CityscapesEvalDataset("/data/disertatie/cityscapes", filenames, HEIGHT, WIDTH, frames_to_load, 4, is_train=False)
        dataloader = DataLoader(dataset, 4, shuffle=False, num_workers=8, pin_memory=True, drop_last=False)
        ...
        gt_depths = os.path.join(splits_dir, conf['evaluation_split'], "gt_depths")
        ...
        with torch.no_grad():
            for i, data in tqdm.tqdm(enumerate(dataloader)):
                ...
                if conf['evaluation_post_process']:
                    # print("post_process *********")
                ...
                pred_disp, _ = disp_to_depth(output, conf['min_depth'], conf['max_depth'])
    ...
    if conf['save_predicted_disparities']:
        print("saving errors")
        if True:
            tag = "mono"
        else:
        ...
        output_path = os.path.join(
            conf['pretrained_models_folder'], "{}_{}_errors.npy".format(tag, conf['evaluation_split']))
    ...
    if conf['save_predicted_disparities']:
        ...
        error_map_path = os.path.join(
            conf['pretrained_models_folder'], "error_{}_split".format(conf['evaluation_split']))
