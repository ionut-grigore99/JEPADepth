...
def evaluate(conf):
    ...
    if get('numpy_disparities_to_evaluate') is None: # optional path to a .npy disparities file to evaluate.
        ...
        if conf.get('model_name').startswith("mambaDepth"):
            ...
            model = VMUNet(
                            ...
                            drop_path_rate=0.2,
                            load_ckpt_path='/data/disertatie/MambaDepth/src/pretrained/vssmbase_dp06_ckpt_epoch_241.pth'
                          )
            model.load_from()
            model.from_pretrained(weights_path='/data/disertatie/MambaDepth/src/tensorboard/train/vmunet/20240219-151719/models/weights_epoch_22/depth_encoder_decoder.pth', device='cuda:6')
        ...
        print("-> Computing predictions with size {}x{} (WxH)".format(conf.get('im_sz')[1], conf.get('im_sz')[0]))

        step = 0
        with torch.no_grad():
            for data in tqdm(dataloader):
                step = step + 1
                input_color = data[("color", 0, 0)].to('cuda:6') # tensor[1, 3, 320, 1024] with values between 0 and 1!
                ...
                if conf.get('model_name').startswith("mambaDepth"):
                    ...
                    output = model(input_color)[("disp", 0)] 
                elif conf.get('model_name').startswith("vmunet"):
                    output = model(input_color)[0]
    ...
    if not disable_median_scaling: # Just for printing purposes
        ratios = np.array(ratios)
        med = np.median(ratios)
        print(" Scaling ratios | med: {:0.3f} | std: {:0.3f}".format(med, np.std(ratios / med)))

    if get('save_predicted_disparities'):
        error_map_path = os.path.join(get('pretrained_models_folder'), "error_{}_split.npy".format(get('evaluation_split')))
